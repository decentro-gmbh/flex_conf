...
class FlexConf {
   ...
   * @param {string} [filepath] - Path to save the config file to, defaults to "[os.tmpdir()]/[namespace].json".
   * @param {string|number} [space] - A String or Number object that's used to insert white space into the output JSON string for readability purposes.
   * @returns {string} Path of the saved config file.
   */
  saveToFile(namespace, filepath, space = null) {
    const configObject = this.store.get(namespace);
    if (!filepath) {
      filepath = path.join(os.tmpdir(), `${namespace}.json`);
    }
    fs.writeFileSync(filepath, JSON.stringify(configObject, null, space));
